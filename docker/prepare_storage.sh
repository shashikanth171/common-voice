...
checkAndCreateBucket () {
    ...
    status=$(curl -X GET http://gonthuka3.swecha.org:8092/storage/v1/b/$1 --write-out "%{http_code}\n" --output /dev/null --silent)
    ...
    then 
        ...
        status=$(curl -X POST -d '{"name":"'$1'"}' http://gonthuka3.swecha.org:8092/storage/v1/b?project=local --write-out "%{http_code}\n" --output /dev/null --silent)
